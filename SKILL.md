...
When both `Loop auto-push: on` and `Auto-commit: on` are configured in CLAUDE.md, hands-free automatically runs `git push origin <current-branch>` at the **end of each iteration** after all auto-commits for that iteration have been made.

Before pushing, announce: `[hands-free] Auto-push: pushing N commit(s) to origin/<branch>`

After a successful push, announce: `[hands-free] Auto-push: pushed successfully`
...
**Push failure handling:** If `git push` fails (e.g., remote rejected, auth failure, network error), fire a HARD STOP — do not silently continue to the next iteration: `[hands-free] HARD STOP — Auto-push failed: <error>. Resolve the issue and run /hands-free resume to continue.`
...
### Loop Skip-if-Unchanged

When `Loop skip-if-unchanged: on` is set in CLAUDE.md, hands-free checks the working tree at the **start of each iteration**, before any brainstorming, planning, or execution begins. If the working tree is clean, hands-free skips the iteration entirely and advances to the next one:

```
[hands-free] Skip-if-unchanged: working tree clean — skipping iteration N, advancing to next
```

**"Clean" defined:** `git status` reports no modified files, no staged files, and no untracked files. Files ignored by `.gitignore` are not considered (i.e., the check uses `git status --porcelain` which respects `.gitignore`).

The skipped iteration is logged in the session log as `[loop-skip] iteration N — working tree clean`.

Combining `Loop skip-if-unchanged: on` with `Loop max failures` or stall detection provides comprehensive coverage: skip-if-unchanged handles the "nothing to do" case while stall detection handles the "stuck in a loop" case.
...
### Loop Pre-Iteration Hook

When `Loop pre-iteration: <command>` is set in CLAUDE.md, hands-free runs the specified shell command at the **start of each iteration**, before any brainstorming, planning, or execution begins. This is useful for pulling fresh data, checking external dependencies, or resetting environment state.

Announce before running: `[hands-free] Pre-iteration hook: running '<command>'`

The same cwd-scope auto-pass rules and HARD STOP rules that apply to all shell commands apply here. If the command fails (non-zero exit code), hands-free fires a HARD STOP and does not proceed with the iteration:

`[hands-free] HARD STOP — pre-iteration hook failed: <error>. Fix the hook and run /hands-free resume to continue.`

After `/hands-free resume`, the hook runs again at the start of the next iteration.
...
When `Loop iteration summary: on` is set in CLAUDE.md, hands-free outputs a brief structured summary at the end of each iteration (including skipped, paused, and hard-stop iterations):

```
[hands-free] Iteration N summary:
  Status:   completed | skipped | paused | hard-stop
  Commits:  N (e.g., 2 new commits)
  Tests:    N passing / N failing | indeterminate
  Warnings: none | [list: cooldown, stall-detected, max-failures, etc.]
```

The summary is output to the conversation — it is not written to a file. To persist summaries across sessions, use `Loop notes: on` instead (which writes to `.claude/loop-notes.md`). Combining both directives is supported.
...
When `Loop quiet mode: on` is set in CLAUDE.md, hands-free suppresses routine per-iteration announcements to reduce conversation noise during long-running loops.

**Suppressed in quiet mode:**
- `[hands-free] Iteration N/M — ...` iteration-start status line
- `[hands-free] Cooldown: waiting Xs before next iteration`
- `[hands-free] Backoff: waiting Xs after N consecutive failures`
- `[auto-commit] ...` per-iteration commit messages
- `[hands-free] Auto-push: pushed successfully` push success confirmations

**Always printed regardless of quiet mode:**
- `[hands-free] Warning: ...` lines (stall detection, max-failures, final-3-iterations)
- HARD STOP announcements (including auto-push failure)
- Mandatory review checkpoint blocks
- `[hands-free] LOOP AUTO-STOP: ...` auto-stop condition messages
- Completion promise output (when the loop terminates)
- Pre-push announcements (`[hands-free] Auto-push: pushing N commit(s) to origin/<branch>`) — preserved so push failures always have context
- Iteration-count warnings (final 3 remaining, final 2, final 1 pause)

**Session log:** All events are still captured in `/hands-free log` regardless of quiet mode — quiet mode only affects what is printed to the conversation.

**`/hands-free status`** shows `Loop quiet mode: on` when the directive is active.

**Default:** off — all routine per-iteration announcements are printed normally.

### Loop Abort on Regression

When `Loop abort on regression: on` is set in CLAUDE.md, hands-free monitors the failing test count across iterations and fires a HARD STOP whenever the count increases from one iteration to the next.

**How it works:** At the end of each iteration, hands-free records the number of failing tests. At the start of the next iteration — before any work begins — it compares the current count to the previous iteration's count. If the current count is greater, it fires:

`[hands-free] HARD STOP — Regression detected: N failing tests (was M in previous iteration). Run /hands-free resume to continue anyway.`

**First iteration:** The first iteration has no prior baseline to compare against, so the comparison is skipped and the iteration proceeds normally. The failing count from the first iteration becomes the baseline for the second.

**After `/hands-free resume`:** When the user resumes after a regression HARD STOP, the current failing count becomes the new baseline. Subsequent iterations compare against the resumed state, not the pre-regression count.

**Compatibility:** Works alongside `Loop max failures` — both directives can be active simultaneously. Regression abort fires when count increases; max-failures fires when the count stays high for N consecutive iterations. They address complementary failure patterns.
...
When `Loop max changes: N` is set in CLAUDE.md, hands-free checks the number of files changed at the end of each iteration after all edits are complete but before auto-commit.

**File count method:** `git status --short` line count — includes staged changes, unstaged changes, and new untracked files. Excludes `.gitignore`d files.

**Check timing:** After all edits complete, before auto-commit.

**HARD STOP trigger:** If the file count exceeds N:

```
[hands-free] HARD STOP — Max changes exceeded: N files changed in this iteration (limit: M). Run /hands-free resume to commit and continue anyway.
```

**After `/hands-free resume`:** The iteration proceeds to auto-commit despite exceeding the limit. The limit is not re-checked for the same iteration.
...
### Loop Skip on No Changes

When `Loop skip on no changes: warn` is set in CLAUDE.md, hands-free emits a visible notice when an iteration ends with no file changes, instead of silently skipping auto-commit.

**No-changes detection:** `git status --short` produces empty output — no staged changes, no unstaged changes, no untracked files.

**Notice:**

```
[hands-free] Loop skip: no changes produced in this iteration
```

**Session log:** The skip event is always captured in the session log regardless of this setting.

**Quiet mode interaction:** When `Loop quiet mode: on` is active, this notice is suppressed (quiet mode governs routine per-iteration announcements).

**Default:** absent — silent skip (existing behavior unchanged).

### Loop Branch Guard

When `Loop branch guard: <branch>` is set in CLAUDE.md, hands-free checks the current git branch at the start of each iteration, before any work begins. If the branch does not match the specified value, a HARD STOP is fired.

**Branch detection:** `git branch --show-current`

**HARD STOP trigger:** If the current branch does not match the required branch:

```
[hands-free] HARD STOP — Branch guard: current branch is '<actual>' but Loop branch guard requires '<expected>'. Switch to the correct branch and run /hands-free resume.
```

**Detached HEAD:** If the repo is in detached HEAD state, the same HARD STOP fires with the actual branch shown as `(detached HEAD)`.

**Default:** absent — no branch check is performed.

### Loop Pre-Iteration Hook

When `Loop pre-iteration hook: <cmd>` is set in CLAUDE.md, hands-free runs the specified shell command at the start of each iteration, before the build state health check and any other work.

**Timing:** Runs first in each iteration — before build health check, before editing any files.

**Exit code semantics:** Exit code 0 = proceed with the iteration. Non-zero = HARD STOP:

```
[hands-free] HARD STOP — Pre-iteration hook failed (exit N): <cmd>. Run /hands-free resume to retry or skip the hook for this iteration.
```

**After `/hands-free resume`:** The iteration proceeds without re-running the hook. The hook is not re-checked for the same iteration.

**Scope:** The command is run with cwd-scope classification rules (same as other shell commands in auto-pass rules).

**Default:** absent — no pre-iteration hook is run.

### Loop Post-Iteration Hook

When `Loop post-iteration hook: <cmd>` is set in CLAUDE.md, hands-free runs the specified shell command at the end of each iteration, after auto-commit and after auto-push (if `Loop auto-push: on`).

**Timing:** Runs at the very end of the iteration — after auto-commit and after auto-push. If auto-commit is off or no changes were committed this iteration, the hook still runs.

**Exit code semantics:** Exit code 0 = proceed to the next iteration. Non-zero = HARD STOP:

```
[hands-free] HARD STOP — Post-iteration hook failed (exit N): <cmd>. Run /hands-free resume to continue to the next iteration.
```

**After `/hands-free resume`:** The loop continues to the next iteration. The hook is not re-run.

**Scope:** The command is run with cwd-scope classification rules.
...
| `Loop auto-push: on/off` | `Loop auto-push: on` | When `on` (and `Auto-commit: on`), runs `git push origin <branch>` after each iteration's commits complete; push failure fires a HARD STOP (not silent continue); skipped on loop-skip, HARD STOP, no new commits, or `Auto-commit: off`; default: `off` |
...
| `Loop skip on no changes: warn` | `Loop skip on no changes: warn` | When set to `warn`, emits a visible notice (`[hands-free] Loop skip: no changes produced in this iteration`) instead of silently skipping auto-commit when an iteration produces no file changes; session log always captures the skip event regardless; suppressed when `Loop quiet mode: on`; absent by default (silent skip) |
